...
from inv_kine import solve_ik, pick_angles
...
class testqt5(Ui_MainWindow):
    ...
    def initState(self):
        ...
        y=0
        z=20
        self.hslider1.setProperty("value", x)
        self.hslider2.setProperty("value", y)
        self.hslider3.setProperty("value", z)
    ...
    def s1label(self):
        self.labels1.setText("X value: "+str(self.hslider1.value())+u'\N{DEGREE SIGN}')
        self.draw2()
    def s2label(self):
        self.labels2.setText("Y value: "+str(self.hslider2.value())+u'\N{DEGREE SIGN}')
        self.draw2()
    def s3label(self):
        self.labels3.setText("Z value: "+str(self.hslider3.value())+u'\N{DEGREE SIGN}')
        self.draw2() 
    ...
    def draw2(self): 
        x=self.hslider1.value()
        y=self.hslider2.value()
        z=self.hslider3.value()
        print(x, y, z)
        j1, q_2_a, q_2_b, q_3_a, q_3_b = solve_ik(x, y, z)
        j2, j3 = pick_angles(q_2_a, q_2_b, q_3_a, q_3_b)
        
        j=fk.dh_par(np.rad2deg(j1),np.rad2deg(j2),np.rad2deg(j3))
        Tm=fk.dh_kine(j)
        ee=fk.el_xyzpos(Tm)
        p0,p1,p2,p3,p4,p5=fk.el_pos2base(Tm)
        ...
        self.MplWidget.canvas.axes.set_ylim([-50,50])
        self.MplWidget.canvas.axes.set_zlim([0,40])

        # q_1, q_2_a, q_2_b, q_3_a, q_3_b = solve_ik(x4, y4, z4)

        # assert np.isclose(np.rad2deg(q_1), j1), "joint1 differs"
        # print(f'joint2: {j2}, ests: {np.rad2deg(q_2_b)}, {np.rad2deg(q_2_a)}')
        # print(f'joint2: {j3}, ests: {np.rad2deg(q_3_b)}, {np.rad2deg(q_3_a)}')
        # print('')
